...
function DataGrid(props, ref) {
  ...
  var headerRowHeight =
    rawHeaderRowHeight ?? (typeof height === "number" ? 24 : 24); //Need to added--24 instaed of 35
  ...
  const arr = rowArray.slice();
  const mapToPair = () => {
    const result = arr.map((obj) => {
      const res = [];
      res.push(obj.cellWidth);
      return res;
    });
    return result;
  };
  ...
  const arr4 = rawColumns.slice();
  var arr5 = arr2.slice();
  const rawColumnsWithCellWidth = arr4.map((a, index) => {
    const completeObject = Object.assign({}, a);
    completeObject.cellWidth =
      Number(arr5.filter((b, indexx) => index === indexx)) - 1;
  ...
  const {
    ...
    rawColumns,
    rawColumnsWithCellWidth, //need to be added
    columnWidths,
    scrollLeft,
    viewportWidth: totalViewportWidth, //need to be added
    defaultColumnOptions,
    rawGroupBy: rowGrouper ? rawGroupBy : undefined,
    enableVirtualization,
  });
  console.log("layoutCssVars", lastFrozenColumnIndex, totalFrozenColumnWidth);
  ...
  const {
    ...
    columnWidths,
    scrollLeft,
    viewportWidth: totalViewportWidth,
    defaultColumnOptions,
    rawGroupBy: rowGrouper ? rawGroupBy : undefined,
    enableVirtualization,
  });
  ...
  console.log("ff", templateColumns);
  ...
  function getViewportRows() {
    ...
    for (
      ...
      console.log("summaryRowHeight", summaryRowHeight);
      if (isGroupRow(row)) {
        ({ startRowIndex } = row);
  ...
  let templateRows = `${rowHeight}px`; //need to be added
              ...
              {rows.length === 0 && noRowsFallback ? (
                ...
                <>
                  {topSummaryRows?.map((row, rowIdx) => {
                    ...
                    return (
                      <SummaryRow
                        ...
                        viewportColumns={getRowViewportColumnss(summaryRowIdx)}
                        lastFrozenColumnIndex={lastFrozenColumnIndex}
                        selectedCellIdx={
                  ...
                  <RowSelectionChangeProvider value={selectRowLatest}>
                    {getViewportRows(summaryRowHeight)}
                  </RowSelectionChangeProvider>
